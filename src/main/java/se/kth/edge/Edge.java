...
public class Edge {
    ...
    public void keyArrival(long kid, long time, int edgeId) {
        ...
        float estimatedRate = wMonitor.getExpectedKeyArrivalRate(kid);
        if (coordinators.containsKey(kid) && coordinators.get(kid) == eId) {
            cache.insertPriorityKey(kid, time, estimatedRate); // prioritize keys that the edge is its coordinator.
        } else {
